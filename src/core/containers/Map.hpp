...
namespace ari
{
    namespace core
    {
		...
		Map<KEY, VALUE>::Map() :
			minGrow(ARI_CONTAINER_DEFAULT_MIN_GROW),
		...
		template<class KEY, class VALUE> VALUE&
			...
			a_assert((kvp != this->buffer._end()) && (key == kvp->key));    // not found if this triggers
			return kvp->value;
		}
		...
		template<class KEY, class VALUE> const VALUE&
			...
			a_assert_dbg(this->buffer.buf);
			auto kvp = std::lower_bound(this->buffer._begin(), this->buffer._end(), key);
			a_assert_dbg((kvp != this->buffer._end()) && (key == kvp->key));    // not found if this triggers
			return kvp->value;
		}
