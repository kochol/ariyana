...
//------------------------------------------------------------------------------
template <typename TYPE>
Slice<TYPE>::Slice(TYPE *base, int numBaseItems, int sliceOffset, int sliceNumItems) : basePtr(base),
                                                                                       baseSize(numBaseItems),
                                                                                       offset(sliceOffset),
                                                                                       num((sliceNumItems == EndOfRange) ? numBaseItems : sliceNumItems)
{
    a_assert_dbg(basePtr && (offset >= 0) && (num >= 0) && ((offset + num) <= baseSize));
}

//------------------------------------------------------------------------------
template <typename TYPE>
Slice<TYPE>::Slice(const Slice &rhs) : basePtr(rhs.basePtr), baseSize(rhs.baseSize), offset(rhs.offset), num(rhs.num)
{
    // empty
}
...
//------------------------------------------------------------------------------
template <typename TYPE>
void Slice<TYPE>::operator=(const Slice &rhs)
{
...
{
    a_assert_dbg(this->basePtr && (index >= 0) && (index < this->num));
...
//------------------------------------------------------------------------------
template <typename TYPE>
void Slice<TYPE>::SetSize(int numSliceItems)
{
    a_assert_dbg((numSliceItems >= 0) && ((this->offset + numSliceItems) <= this->baseSize));
    this->num = numSliceItems;
}

//------------------------------------------------------------------------------
template <typename TYPE>
int Slice<TYPE>::Offset() const
{
    return this->offset;
}
...
//------------------------------------------------------------------------------
template <typename TYPE>
void Slice<TYPE>::SetOffset(int sliceOffset)
{
    a_assert_dbg((sliceOffset >= 0) && ((sliceOffset + this->num) <= this->baseSize));
    this->offset = sliceOffset;
}

//------------------------------------------------------------------------------
template <typename TYPE>
void Slice<TYPE>::Move(int delta)
{
    a_assert_dbg((this->offset + delta) >= 0);
    a_assert_dbg((this->offset + delta + this->num) <= this->baseSize);
    this->offset += delta;
}

//------------------------------------------------------------------------------
template <typename TYPE>
void Slice<TYPE>::FillGap(int gapOffset, int gapSize)
{
    a_assert((gapOffset >= 0) && (gapSize > 0) && ((gapOffset + gapSize) <= this->baseSize));
