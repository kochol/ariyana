...
namespace ari
{
	namespace _priv
	{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		elementBuffer<TYPE>::elementBuffer() : buf(nullptr),
			cap(0),
			start(0),
			end(0)
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		elementBuffer<TYPE>::elementBuffer(const elementBuffer& rhs) : buf(nullptr),
			cap(0),
			start(0),
			end(0)
		{
			if (rhs.buf)
			{
				...
				a_assert_range_dbg(rhs.start, rhs.cap);
				a_assert_dbg((rhs.start + rhs.size()) <= rhs.cap);
				a_assert_range_dbg(this->start, this->cap);
				a_assert_dbg((this->start + this->size()) <= this->cap);
				copyConstruct(&rhs.buf[rhs.start], &this->buf[this->start], rhs.size());
				this->end = this->start + rhs.size();
				a_assert_dbg(rhs.end <= rhs.cap);
			}
		}

		//------------------------------------------------------------------------------
		template <class TYPE>
		elementBuffer<TYPE>::elementBuffer(elementBuffer&& rhs) : buf(rhs.buf),
			cap(rhs.cap),
			start(rhs.start),
			end(rhs.end)
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::operator=(const elementBuffer<TYPE>& rhs)
		{
			if (&rhs != this)
			{
				this->destroy();
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::operator=(elementBuffer<TYPE>&& rhs)
		{
			if (&rhs != this)
			{
				this->destroy();
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		int elementBuffer<TYPE>::frontSpare() const
		{
			return this->start;
		}
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		int elementBuffer<TYPE>::backSpare() const
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		int elementBuffer<TYPE>::spare() const
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		int elementBuffer<TYPE>::size() const
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		int elementBuffer<TYPE>::capacity() const
		{
			return this->cap;
		}
		...
		{
			a_assert_dbg((index >= 0) && (index < this->size()));
			a_assert_dbg(this->buf);
			a_assert_range_dbg(this->start + index, this->cap);
		...
		{
			a_assert((this->start != this->end) && this->buf);
			a_assert_range_dbg(this->start, this->cap);
		...
		{
			a_assert((this->start != this->end) && this->buf);
			a_assert_range_dbg(this->end - 1, this->cap);
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::alloc(int newCapacity, int newStart)
		{
			a_assert_dbg(newCapacity > 0);
			if (this->cap == newCapacity)
			{
				return;
			}
			const int curSize = this->size();
			a_assert_dbg((newStart + curSize) <= newCapacity);
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::destroy()
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::destroyElement(TYPE* elm)
		{
			elm->~TYPE();
		}
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		bool elementBuffer<TYPE>::overlaps(const TYPE* from, const TYPE* to, int num)
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::copyConstruct(const TYPE* from, TYPE* to, int num)
		{
			a_assert_dbg(!overlaps(from, to, num));
			for (int i = 0; i < num; i++)
			{
				new (to++) TYPE(*from++);
			}
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::copyAssign(const TYPE* from, TYPE* to, int num)
		{
			a_assert_dbg(!overlaps(from, to, num));
			for (int i = 0; i < num; i++)
			{
		...
		template <class TYPE>
		template <class... ARGS>
		void elementBuffer<TYPE>::emplaceBack(ARGS&& ... args)
		{
			// NOTE: this will fail if there is no spare space at the back,
			// use insert(size(), elm) which will move towards front if possible
			a_assert_dbg(this->buf);
		...
		template <class TYPE>
		template <class... ARGS>
		void elementBuffer<TYPE>::emplaceFront(ARGS&& ... args)
		{
			// NOTE: this will fail if there is no spare space at the front,
			// use insert(0, elm) which will move towards back if possible
			a_assert_dbg(this->buf && (this->start > 0) && (this->start <= this->cap));
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::moveEraseFront(int index)
		{
			// erase a slot by moving elements from the front
			a_assert_dbg(this->buf && (index >= 0) && (index < this->size()));
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::moveEraseBack(int index)
		{
			...
			else
			{
				...
				if (this->frontSpare() > this->backSpare())
				{
					...
					else
					{
						...
						a_assert_dbg(this->end < this->cap);
						return this->moveInsertBack(index);
					}
				}
				else
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::insert(int index, const TYPE& elm)
		{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::insert(int index, TYPE&& elm)
		{
			bool slotConstructed = true;
			TYPE* ptr = this->prepareInsert(index, slotConstructed);
			if (slotConstructed)
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::erase(int index)
		{
			const int size = this->size();
			a_assert_dbg(this->buf && (index >= 0) && (index < size));
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::eraseSwap(int index)
		{
			const int size = this->size();
			a_assert_dbg(this->buf && (index >= 0) && (index < size));
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::eraseSwapBack(int index)
		{
			const int size = this->size();
			a_assert_dbg(this->buf && (index >= 0) && (index < size));
			if (index == (size - 1))
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::eraseSwapFront(int index)
		{
			a_assert_dbg(this->buf && (index >= 0) && (index < this->size()));
			if (0 == index)
			{
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		void elementBuffer<TYPE>::eraseRange(int index, int num)
		{
			a_assert_dbg(this->buf && (index >= 0) && ((index + num) <= this->size()) && (num >= 0));
			if (0 == num)
			{
				return;
			}
			...
			{
				a_assert_range_dbg(i, this->cap);
				a_assert_range_dbg(i + num, this->cap);
			...
			this->end -= num;
			a_assert_dbg(this->end >= this->start);
		}

		//------------------------------------------------------------------------------
		template <class TYPE>
		TYPE elementBuffer<TYPE>::popBack()
		{
			a_assert_dbg(this->buf && (this->end > this->start));
			a_assert_dbg((this->end > 0) && (this->end <= this->cap));
		...
		//------------------------------------------------------------------------------
		template <class TYPE>
		TYPE elementBuffer<TYPE>::popFront()
		{
			a_assert_dbg(this->buf && (this->start < this->end));
			a_assert_range_dbg(this->start, this->cap);
