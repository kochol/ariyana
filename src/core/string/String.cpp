...
String::String(const char* ptr, int startIndex, int endIndex) {
    a_assert(nullptr != ptr);
    if (EndOfString == endIndex) {
        endIndex = int(std::strlen(ptr));
    }
    a_assert(startIndex < endIndex);
...
void
String::destroy() {
    a_assert(0 != this->data);
    a_assert(0 == this->data->refCount);
